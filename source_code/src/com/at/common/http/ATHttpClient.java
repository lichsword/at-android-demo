...
public class ATHttpClient {
	...
	public ATHttpResponse get(String url, boolean autoRedirect) throws Exception {
		return get(url, null, null, autoRedirect);
	}
	...
	public ATHttpResponse get(String url, boolean autoRedirect, ATHttpParameter... queryParams) throws Exception {
		return get(url, queryParams, null, autoRedirect);
	}
	...
	public ATHttpResponse get(String url, ATHttpParameter[] queryParams, ATHttpParameter[] heads, boolean autoRedirect)
			throws Exception {
		if (queryParams != null && queryParams.length > 0) {
			String query = encodeParameters(queryParams);
			url += "?" + query;
		}
		return httpRequest(url, null, heads, autoRedirect);
	}

	protected ATHttpResponse httpRequest(String url, ATHttpParameter[] postParams, ATHttpParameter[] heads)
			throws Exception {
		boolean autoRedirect = true;// 自动处理重定向
		return httpRequest(url, postParams, heads, autoRedirect);
	}
		...
		for (retriedCount = 0; retriedCount < retry; retriedCount++) {
			...
			try {
				...
				try {
					...
					con.setInstanceFollowRedirects(autoRedirect);
			...
			try {
				// log("Sleeping " + retryIntervalMillis +" millisecs for next
				// retry.");
	...
	public static String encodeParameters(ATHttpParameter[] postParams) {
		...
		for (ATHttpParameter ATHttpParameter : postParams) {
			...
			buf.append(URLEncoder.encode(ATHttpParameter.name)).append("=")
					.append(URLEncoder.encode(ATHttpParameter.value));
		}
		return buf.toString();
